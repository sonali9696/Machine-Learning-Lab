...
class Eigenfaces(object):                                                       # *** COMMENTS ***
    ...
    train_count = 6                                                       # number of faces used for training
    test_faces_count = 4                                                        # number of faces used for testing

    l = train_count * people_total                                        # training images count
    ...
    def __init__(self, _faces_dir = '.', _energy = 0.85):
        ...
        cur_img = 0
        for face_id in xrange(1, self.people_total + 1):

            training_ids = random.sample(range(1, 11), self.train_count)  # the id's of the 6 random training images
    ...
    def classify(self, path_to_img):
        ...
        return (closest_face_id / self.train_count) + 1                   # return the faceid (1..40)
    ...
    def evaluate(self):
        ...
        test_count = self.test_faces_count * self.people_total                   # number of all AT&T test images/faces
        test_correct = 0
        for face_id in xrange(1, self.people_total + 1):
    ...
    def evaluate_celebrities(self, celebrity_dir='.'):
        ...
        for img_name in os.listdir(celebrity_dir):                              # go through all the celebrity images in the folder
            ...
            for top_id in top5_ids:
                face_id = (top_id / self.train_count) + 1                 # getting the face_id of one of the closest matches
                subface_id = self.training_ids[face_id-1][top_id % self.train_count]           # getting the exact subimage from the face
