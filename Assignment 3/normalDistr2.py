...
for num in n:
	...
	for i in test:
		...
		#c1 = ((test_data - mean[0]) @ np.linalg.inv(cov1)) @ (test_data - mean[0]).T;
		#c1 = np.dot(np.dot(np.asmatrix(test_data - mean[0]), np.asmatrix(np.linalg.inv(cov1))),np.asmatrix(test_data - mean[0]).T);
		c2 = np.matmul(np.matmul((test_data - u2), np.linalg.inv(E1)),(test_data - u2).T);
		p1 = 1/math.sqrt(np.linalg.det(cov1)) * math.exp(-0.5*c1) 
		p2 = 1/math.sqrt(np.linalg.det(cov2)) * math.exp(-0.5*c2)
		if(P1_est*p1 >= P2_est*p2):
			calc_cls = 1
